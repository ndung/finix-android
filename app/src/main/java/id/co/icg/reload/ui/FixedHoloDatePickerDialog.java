...
public class FixedHoloDatePickerDialog extends DatePickerDialog {
    public FixedHoloDatePickerDialog(Context context, OnDateSetListener callBack, int year, int monthOfYear, int dayOfMonth) {
                ...
                for (int i = 0; i < llSecond.getChildCount(); i++) {
                    ...
                    for (Field pf : pickerFields) {
                        if (pf.getName().equals("mSelectionDivider")) {
                            ...
                            try {
                                pf.set(picker, new ColorDrawable(picker.getResources().getColor(R.color.colorAccent)));
